...
const RUNTIME_PACKAGE_PREFIX = '@substrate/runtime-';
...
export class Environment {
  ...
  async close(): Promise<EnvironmentInfo> {
    assertEnvironmentId(this.environmentInfo.id);
    const workers = this.processes.filter((managed) => managed.name !== 'substrate-runtime-host');
    const hosts = this.processes.filter((managed) => managed.name === 'substrate-runtime-host');
...
async function cleanupPartialCreate(
  ...
  const workers = processes.filter((managed) => managed.name !== 'substrate-runtime-host');
  const hosts = processes.filter((managed) => managed.name === 'substrate-runtime-host');
  for (const managed of [...workers].reverse()) {
    await terminateProcess(managed);
  }
  try {
...
type RequiredPolicyConfig = {
  readRoots: string[];
  writeRoots: string[];
  process: Required<Omit<ProcessPolicyConfig, 'maxProcesses'>> & Pick<ProcessPolicyConfig, 'maxProcesses'>;
...
async function materializeConfig(config: EnvironmentConfig): Promise<RequiredRuntimeConfig> {
  ...
  const worker = workerConfig.kind === 'external'
    ...
    : {
        kind: 'managed' as const,
        id: optionalIdentifierString(workerConfig.id, 'worker.id') ?? 'substrate-runtime-js-worker',
  ...
  const queueDir = explicitQueueDir ?? await mkdtemp(join(tmpdir(), 'substrate-runtime-queue-'));
  const sdkCreatedQueueDir = explicitQueueDir === undefined || !existsSync(queueDir);
  const stateDir = hostConfig.kind === 'managed'
    ? explicitStateDir ?? await mkdtemp(join(tmpdir(), 'substrate-runtime-state-'))
    : undefined;
...
function requiredQueueDir(config: RequiredRuntimeConfig): string {
  ...
  throw new Error('Substrate environment has no file queue');
}
...
async function waitForResult(
  ...
  while (Date.now() - started < timeoutMs) {
    ...
    if (failed) {
      ...
      throw new Error(`Substrate invocation failed: ${failed.error.message}`);
    }
    await sleep(10);
  }

  throw new Error(`Timed out waiting for Substrate invocation ${invocationId}`);
}
...
function assertCompletedEnvelopeMatches(
  ...
  if (terminalEventType(completed) !== 'tool.invocation.completed') {
    throw new Error(`Substrate terminal result event type mismatch for invocation ${invocationId}`);
  }
  if (completed.invocationId !== invocationId || result.invocationId !== invocationId) {
    throw new Error(`Substrate terminal result invocation mismatch for invocation ${invocationId}`);
  }
  if (completed.sessionId !== sessionId || result.sessionId !== sessionId) {
    throw new Error(`Substrate terminal result session mismatch for invocation ${invocationId}`);
  }
}
...
function assertFailedEnvelopeMatches(
  ...
  if (terminalEventType(failed) !== 'tool.invocation.failed') {
    throw new Error(`Substrate terminal failure event type mismatch for invocation ${invocationId}`);
  }
  if (failed.invocationId !== invocationId) {
    throw new Error(`Substrate terminal failure invocation mismatch for invocation ${invocationId}`);
  }
  if (failed.sessionId !== sessionId) {
    throw new Error(`Substrate terminal failure session mismatch for invocation ${invocationId}`);
  }
  ...
  if (
    ...
    throw new Error(`Substrate terminal failure malformed for invocation ${invocationId}`);
  }
...
function assertTerminalLeaseMaterial(
  ...
  if (
    ...
    throw new Error(`Substrate terminal ${terminalKind} missing lease material for invocation ${invocationId}`);
  }
}
...
async function terminalMatchesClaim(
  ...
  try {
    ...
    throw new Error(`Substrate terminal claim missing or malformed for invocation ${invocationId}: ${(error as Error).message}`);
  }
...
function runtimeBinaryName(): string {
  return process.platform === 'win32' ? 'substrate-runtime.exe' : 'substrate-runtime';
}
...
function runtimeSpawnError(binaryPath: string, error: unknown): Error {
  ...
  return new Error(
    `Unable to start the Substrate runtime binary at "${binaryPath}".` +
    `${reason}\nInstall a package that includes the runtime binary, install the ` +
    '`substrate-runtime` CLI on PATH, or pass binaryPath/SUBSTRATE_RUNTIME_BIN.',
  );
}
...
async function waitForHealth(baseUrl: string, timeoutMs: number): Promise<void> {
  ...
  throw new Error(`Timed out waiting for Substrate host at ${baseUrl}`);
}
...
async function postJson(url: string, body: unknown): Promise<unknown> {
  ...
  if (!response.ok) {
    throw new Error(`Substrate host returned ${response.status}: ${await cappedResponseText(response, MAX_HTTP_ERROR_BODY_BYTES)}`);
  }
