...
const repoDebugSubstrateBinary = join(
  repoRoot,
  'target',
  'debug',
  process.platform === 'win32' ? 'substrate-runtime.exe' : 'substrate-runtime',
);
const repoReleaseSubstrateBinary = join(
  repoRoot,
  'target',
  'release',
  process.platform === 'win32' ? 'substrate-runtime.exe' : 'substrate-runtime',
...
if (!process.env.SUBSTRATE_RUNTIME_BIN) {
  if (existsSync(repoDebugSubstrateBinary)) {
    process.env.SUBSTRATE_RUNTIME_BIN = repoDebugSubstrateBinary;
  } else if (existsSync(repoReleaseSubstrateBinary)) {
    process.env.SUBSTRATE_RUNTIME_BIN = repoReleaseSubstrateBinary;
  }
}
...
function runtimeSidecarPackageName(): string | undefined {
  ...
  return `@substrate/runtime-${process.platform}-${process.arch}`;
}
...
describe('Environment file queue validation', () => {
  ...
  test('submit result parser preserves structured tool errors', async () => {
    const root = await mkdtemp(join(tmpdir(), 'substrate-runtime-js-test-'));
    cleanup.push(root);
  ...
  test('create rejects malformed config values before spawning processes', async () => {
    ...
    for (const baseUrl of [
      'file:///tmp/substrate-runtime',
      'http:///tmp/substrate-runtime',
    ...
    await expect(Environment.create({
      backend: { kind: 'file', queueDir: join(tmpdir(), 'substrate-runtime-js-should-not-exist') },
      host: { kind: 'managed', stateDir: join(tmpdir(), 'substrate-runtime-js-state-should-not-exist') },
      worker: { kind: 'external' },
      workspace: { kind: 'existing', root: 'relative-workspace' },
    })).rejects.toThrow('workspace.root must be absolute');

    const symlinkRoot = await mkdtemp(join(tmpdir(), 'substrate-runtime-js-workspace-parent-'));
  ...
  test('create does not resolve the runtime binary from the repo target directory', async () => {
    ...
    const previous = process.env.SUBSTRATE_RUNTIME_BIN;
    try {
      delete process.env.SUBSTRATE_RUNTIME_BIN;
      const env = await Environment.create({
        backend: { kind: 'file', queueDir },
        host: { kind: 'http', baseUrl: `http://127.0.0.1:${server.port}/` },
        worker: { kind: 'external' },
      ...
      try {
        ...
        expect(basename(config.binaryPath)).toBe(process.platform === 'win32' ? 'substrate-runtime.exe' : 'substrate-runtime');
      } finally {
        await env.close();
      }
    } finally {
      if (previous === undefined) {
        ...
        process.env.SUBSTRATE_RUNTIME_BIN = previous;
      }
      server.stop(true);
    }
  });
  ...
  test('create resolves a bundled package runtime binary', async () => {
    ...
    const bundledBinary = join(packageBinDir, process.platform === 'win32' ? 'substrate-runtime.exe' : 'substrate-runtime');
  ...
  test('create resolves a platform runtime sidecar package', async () => {
    ...
    const sidecarBinary = join(sidecarBinDir, process.platform === 'win32' ? 'substrate-runtime.exe' : 'substrate-runtime');
  ...
  test('close waits for managed child processes to exit after termination', async () => {
    ...
    const env = new (Environment as unknown as new (
      ...
      [{ name: 'substrate-runtime-worker', process: fakeProcess }],
    );
...
describe('workspace artifact materialization', () => {
  test('rejects ambiguous file artifact URIs before reading', async () => {
    const root = await mkdtemp(join(tmpdir(), 'substrate-runtime-js-artifact-'));
    cleanup.push(root);
