...
def substrate_runtime_binary() -> str:
    debug_binary = Path(__file__).resolve().parents[3] / "target" / "debug" / "substrate-runtime"
    if "SUBSTRATE_RUNTIME_BIN" not in os.environ and debug_binary.exists():
        return str(debug_binary)
    return os.environ.get(
        "SUBSTRATE_RUNTIME_BIN",
        str(Path(__file__).resolve().parents[3] / "target" / "release" / "substrate-runtime"),
    )
...
class EnvironmentTests(unittest.TestCase):
    ...
    def test_runtime_binary_resolution_respects_explicit_overrides(self) -> None:
        self.assertEqual(_resolve_binary_path("/opt/substrate/substrate-runtime"), "/opt/substrate/substrate-runtime")

        with patch.dict(os.environ, {"SUBSTRATE_RUNTIME_BIN": "/opt/substrate/env-substrate-runtime"}):
            self.assertEqual(_resolve_binary_path(None), "/opt/substrate/env-substrate-runtime")
    ...
    def test_materialize_config_rejects_malformed_values(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "workspace.root must be absolute"):
            _materialize_config(
                binary_path="substrate-runtime",
                backend={"kind": "file"},
                host={"kind": "managed", "stateDir": "/tmp/substrate-python-state-should-not-exist"},
                worker={"kind": "external"},
    ...
    def test_create_cleans_up_started_resources_on_failure(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(terminated, ["substrate-host"])
    ...
    def test_create_destroys_session_before_stopping_managed_host_on_worker_start_failure(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(events, ["destroy:env_partial", "terminate:substrate-host"])
    ...
    def test_close_stops_managed_processes_before_destroying_session(self) -> None:
        ...
        env = Environment(config, session, [ProcessRef("substrate-worker")])  # type: ignore[list-item]
    ...
    def test_close_cleans_resources_when_destroy_fails(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            env = Environment(config, session, [
                ProcessRef("substrate-host"),
                ProcessRef("substrate-worker"),
            ...
            self.assertEqual(terminated, ["substrate-worker", "substrate-host"])
            self.assertFalse(queue.exists())
            self.assertFalse(state.exists())
    ...
    def test_write_read_edit_with_managed_worker(self) -> None:
        with tempfile.TemporaryDirectory() as workspace:
            with Environment.create(
                binaryPath=substrate_runtime_binary(),
                workspace={"kind": "existing", "root": workspace},
                worker={"kind": "managed", "id": "substrate-python-test-worker", "idleSleepMs": 1},
            ) as env:
                session = env.create_session()
    ...
    def test_export_workspace_materializes_artifact(self) -> None:
        with tempfile.TemporaryDirectory() as workspace, tempfile.TemporaryDirectory() as output_dir:
            with Environment.create(
                binaryPath=substrate_runtime_binary(),
                workspace={"kind": "existing", "root": workspace},
                worker={"kind": "managed", "id": "substrate-python-artifact-worker", "idleSleepMs": 1},
            ) as env:
                session = env.create_session()
                session.submit({
                    "toolName": "Write",
