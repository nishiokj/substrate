...
_RUNTIME_PACKAGE_NAMES = ("substrate_runtime",)
...
class Environment:
    ...
    def close(self) -> EnvironmentInfo:
        _assert_environment_id(self._environment.id)
        worker_processes = [
            managed for managed in self._processes if managed.name != "substrate-host"
        ]
        host_processes = [
            managed for managed in self._processes if managed.name == "substrate-host"
        ]
...
def _cleanup_partial_create(
    ...
    worker_processes = [
        managed for managed in processes if managed.name != "substrate-host"
    ]
    host_processes = [
        managed for managed in processes if managed.name == "substrate-host"
    ]
    ...
    *,
    ...
    if host_kind == "http":
        ...
        if host.get("stateDir") is None:
            state_dir = tempfile.mkdtemp(prefix="substrate-state-")
        else:
    ...
    if backend.get("queueDir") is None:
        queue_dir = tempfile.mkdtemp(prefix="substrate-queue-")
...
def _required_queue_dir(config: _RuntimeConfig) -> str:
    ...
    raise RuntimeError("Substrate environment has no file queue")
...
def _wait_for_result(
    ...
    while time.monotonic() - started < timeout_s:
        ...
        if failed is not None:
            ...
            raise RuntimeError(f"Substrate invocation failed: {message}")
        time.sleep(0.01)

    raise TimeoutError(f"Timed out waiting for Substrate invocation {invocation_id}")
...
def _assert_completed_envelope_matches(
    ...
    if not isinstance(result, Mapping):
        raise RuntimeError(f"Substrate terminal result malformed for invocation {invocation_id}")
    if _terminal_event_type(completed) != "tool.invocation.completed":
        raise RuntimeError(f"Substrate terminal result event type mismatch for invocation {invocation_id}")
    if completed.get("invocationId") != invocation_id or result.get("invocationId") != invocation_id:
        raise RuntimeError(f"Substrate terminal result invocation mismatch for invocation {invocation_id}")
    if completed.get("sessionId") != session_id or result.get("sessionId") != session_id:
        raise RuntimeError(f"Substrate terminal result session mismatch for invocation {invocation_id}")
...
def _assert_failed_envelope_matches(
    ...
    if _terminal_event_type(failed) != "tool.invocation.failed":
        raise RuntimeError(f"Substrate terminal failure event type mismatch for invocation {invocation_id}")
    if failed.get("invocationId") != invocation_id:
        raise RuntimeError(f"Substrate terminal failure invocation mismatch for invocation {invocation_id}")
    if failed.get("sessionId") != session_id:
        raise RuntimeError(f"Substrate terminal failure session mismatch for invocation {invocation_id}")
    ...
    if (
        ...
        raise RuntimeError(f"Substrate terminal failure malformed for invocation {invocation_id}")
...
def _assert_terminal_lease_material(
    ...
    if (
        ...
        raise RuntimeError(
            f"Substrate terminal {terminal_kind} missing lease material for invocation {invocation_id}"
        )
...
def _terminal_matches_claim(
    ...
    try:
        ...
        raise RuntimeError(
            f"Substrate terminal claim missing or malformed for invocation {invocation_id}: {exc}"
        ) from exc
...
def _resolve_binary_path(binary_path: str | None) -> str:
    if binary_path:
        return binary_path
    env_binary = os.environ.get("SUBSTRATE_RUNTIME_BIN")
    if env_binary:
        return env_binary
...
def _runtime_binary_name() -> str:
    return "substrate-runtime.exe" if os.name == "nt" else "substrate-runtime"
...
def _spawn_process(binary_path: str, args: list[str], name: str) -> _ManagedProcess:
    try:
        ...
        raise RuntimeError(
            f'Unable to start the Substrate runtime binary at "{binary_path}". '
            "Install substrate-runtime, install the `substrate-runtime` CLI on PATH, "
            "or pass binaryPath/SUBSTRATE_RUNTIME_BIN."
        ) from exc
...
def _wait_for_health(base_url: str, timeout_ms: int) -> None:
    ...
    while time.monotonic() - started < timeout_s:
        try:
            ...
            pass
        time.sleep(0.025)
    raise TimeoutError(f"Timed out waiting for Substrate host at {base_url}")
